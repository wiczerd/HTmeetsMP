function calresid = cal_undevd(LamAm,Natarget,utarget)
...
if(exitflag <0)
	...
	calresid_v(1)	= (Natarget - theeconomy(1)).^2;
	calresid_v(2)	= (utarget - theeconomy(2)/(1-theeconomy(1)))^2;
	if(theeconomy(1)>=1-1e-4) calresid_v(1) = calresid_v(1)*20; end%.05*(1/Amf); end
end	

calresid = sum(calresid_v);
%disp(calresid_v)