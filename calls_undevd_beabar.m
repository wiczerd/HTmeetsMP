...
%pos_solwc = @(wcAa) sol_wcAa((atan(wcAa)+pi/2)*Ym/pi,Pa,trans_path(2,:),trans_path(1,2)); % should I use trans_path(1,2) instead of utarget*(1-Natarget)
...
if(exitflag <0)
	...
	for itertau = 1:100
		...
		%[tanw, fval,exitflag,output,J] = fsolve(pos_solwc,tan(w0*pi/Ym-pi/2),options);
		%[excess,theeconomy] = sol_wcAa((atan(tanw)+pi/2)*Ym/pi,Pa,trans_path(2,:),trans_path(1,2));
