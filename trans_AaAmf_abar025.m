...
% clean breaks in from the price series: take out large deviations from the
% polynomial fit (maybe absolute deviations would be best metric?)
...
% 		end
	...
	trans_path(t,:) = trans_economy;
	excess_path(t,:)= excess_trans;
	price_path(t,:) = wcPa_t*.25 + .75*price_path_back(t,:);
end
trans_path(TT,:) = devd_economy;
price_dif = abs(price_path - price_path_back);
if(max(max(price_dif))<1e-4)
	break;
else
	disp('Max deviation:')
	max(max(price_dif))
